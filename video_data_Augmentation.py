...
def build_transform(args):
    ...
    if args.fog:
        transform = A.Compose([
            ...
            A.RandomFog(fog_coef_range=(0.3, 1), alpha_coef=0.1, p=1.0)
        ])
    if args.rain:
...
if __name__ == '__main__':
    args = get_args()
    input_data_dir = '/home/rachel/.cache/huggingface/nextqa/NExTVideo'
    output_data_dir_root = '/home/rachel/projects/leo/restriected_videos/NExTVideo'
    ...
    for root, dirs, files in os.walk(input_data_dir):
        for file in files:
            ...
            if file.endswith(".mp4"):
                if file in os.listdir(output_data_dir):
                    print('file existed~')
                    continue

            # load video as frames
            frames = load_video(input_file_path)
            output_video_path = os.path.join(output_data_dir, file)
            ...
            if args.fgsm:
                ...
                if args.snow or args.brightness or args.downscale or args.noise or args.pepper:
                    save_video(aug_frames, aug_video_path='temp-pepper.mp4')
                    cmd = "ffmpeg -y -i {0} -b:v 1M {1}".format('temp-pepper.mp4', output_video_path)
                    os.system(cmd)
                    os.remove('temp-pepper.mp4')
